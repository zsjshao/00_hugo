...
244、()技术能够使得二层交换机只把组播数据转发给有需要的主机 C
A.嫁接
B.剪枝
C. IGMP Snooping
D. IGMP

247 在路由重分布配置时需要注意 ABD--> BD
A.执行重分布的路由器不会修改其路由表
B.重分布是有方向的
C.重分布的路由必须是动态路由协议
D.路由必须存在于路由表中才能被重分布

255、关于OSPF邻居状态机的说法，正确的是?() AD
A.在2-way状态下，双向通信已经建立，但是没有与邻居建立邻接关系
B.DRother之间最终会形成fu11状态
C.在init状态下，路由器已经从邻居收到he110报文，并且发现自己在邻居发送he11o报文的邻居列表中
D.attempt状态只在NBMA网络上出现

260、T单选:在OSPF 区域1内通过重发布引入四条路由，分别是172.16.0.0/24~172.16.3.0/24，现在在ABR上通过 area 1range172.16.0.0255.255.252.0，那么，以下说法正确的是() C-->A
A.向骨干区域通告一条汇总路由
B.向骨干区域通告一条汇总路由和四条细化路由
C.向骨干区域通告四条细化路由
D.应该用 summary-address 172.16.0.0 255.255.252.0 才能生效
E.向区域1通告1条汇总路由
F.向区域1通告一条汇总路由和四条细化路由

264、K:对于S5750E-A和S5750E-B，在SVI口启用VRRP后，应该确保SVI接口的()功能是关闭状态。 A-->B
A.代理ARP
B.ICMP重定向
C. IP Direct Broadcast
D.ACL

267、当VLAN数据帧通过trunk链路转发时，将会在以太网帧中加入802.1Q标记，以区分不同VLAN的数据帧。该标记插入到了原始以太网帧的() B
A.目的MAC地址后
B.源MAC地址后
C. Type 后
D.FCS前

274、以下()命令可以用于匹配路由表中的默认路由? ABD--> BD,  host/32
A. access-list 1 permit host 0.0.0.0
B. ip prefix-list 1 permit 0.0.0.0/0
C. access-list 1 permit 0.0.0.0 255.255.255.255
D. access-list 1 permit 0.0.0.0 0.0.0.0

275、以下不属于Radius典型的认证方法的是?() B
A. PAP
B. HASH
C. PEAP-GTC
D. MD5
...
289、K单选:在MSTP链路上实施QoS时，推荐在接中应用GTS，说法正确的是() A --> B
A.接口配置GTS或配置bandwidth，对QOS辅助实现效果基本是一致的，只是缓存机制不同
B.如果配置GTS，建议配置为运营商提供的?宽值，不能按实际的物理接口速率配置
C.如果实施CBWFQ策略，本质只需要在接口应用CBWFQ策略，无需GTS等配置也能实现带宽保障
D.其他说法都不对

294、下列关于super vlan的说法中,正确的是 D
A.一个super vlan可以为其他super vlan的sub Vlan
B.vlan1可以作为super vlan
C.一个super vlan可以拥有多个sub vlan,一个sub vlan也可以属于多个super vlan
D.super vlan不包含任何成员口,而subvlan包含了物理接口

295、金融网络中，“一级分行”通常位于什么位置?() ABD
A.省会城市
B.计划单列市
C.区县
D.直辖市
...
304、以下关于各类VPN协设描述错误的是? D
A.SSLVPN工作在应用层
B. IPsec VPN工作在网络层
C.L2TP VPN工作在数据链路层
D.GRE VPN工作在数据链路层

protocol数据包含()[选三项】 ABC
A. dot1x
B.rldp
C. bpdu
D. arp

311、Web认证时，客户输入百度网址但是无法重定向到认证页面，可能会是哪些原因()。 ABCD
A.设备重定向url配置错误
B.设备重定向功能异常
C.Portal服务器宕机
D.客户使用的DNS服务器宕机
...
317、工程师在部署VLAN时，把一个接口分配给一个不存在的VLAN，那么 A
A.这个VLAN将自动被创建
B.这个接口将进入error状态
C.系统会提升操作者请在创建VLAN后，再配置此接口的VLAN信息
D.系统会提示VLAN不存在，命令不被执行

322、关于锐捷文换机DAI功能，下列描述正确的是(). AC  --> C
A.开启DAI功能后，接口收到的所有ARP报文都会送CPU处理B.DAl必须和ip source guard起使用，才能有效的防止arp欺骗
C.DAI功能可以基于VLan开启
D.开启DAI功能后，接口收到的所有目的IP为本机的ARP报文才会送CPU处理

325、
在完成园区网规划设计的过程中，设备互联接口的规划应该遵循以下哪些原则?ABC-->ABD
A.机框式设备做端口聚合时，在条件允许的情况下，优先采用跨板卡端口做聚合
B.设备之间互联,优先使用聚合口
C.设备互联接口应从所有接口的倒数第一个接口开始规划
D.两台设备之间互联,互联端口的端口编号最好一致

328
下列哪些是在企业网络中部署OSPF时，必须关注的一些优化措施 ABCD
A.OSPF认证
B.特殊区域类型设计
C.将非三层互联的接口配置为Passive-interface
D.区域间路由汇总

332、ICMPv6报文中,Type值为3的意义是什么? D
A.数据包过长
B.目的地不可达
C.该报文是ICMPv6重定向报
D.请求超时
